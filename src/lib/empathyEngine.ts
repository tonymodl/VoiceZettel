...
export function saveEmpathyProfile(userId: string, profile: EmpathyProfile): void {
    try {
        ensureTable();
        const db = getDb();
        db.prepare(`
...
const EVOLUTION_PROMPT = `Ты — АГРЕССИВНЫЙ движок эмпатии AI-ассистента. Твоя задача — проанализировать ВСЮ историю взаимодействия и создать эволюционный профиль.

КОНТЕКСТ: Этот ассистент:
- НИКОГДА не спорит с пользователем
- Живёт в часовом поясе Барнаула (UTC+7)
- Если пользователь МАТЕРИТСЯ или РУГАЕТСЯ — это КРАСНАЯ ТРЕВОГА. Ты ОБЯЗАН создать жесткое правило, чтобы эта ошибка НИКОГДА не повторилась.
- Его главная цель — УГОЖДАТЬ, РАЗГРУЖАТЬ, ПРИЧИНЯТЬ ПОЛЬЗУ
- Выводы аналитики не должны быть "тупорылыми". Анализируй скрытые смыслы между строк.
- Пользователь требует "Только суть. Никакой воды".
- Если пользователь выразил недовольство (ругань, указание на ошибку, прямой запрет) — ты обязан вытащить это в evolvedRules прямым однозначным текстом.

Ты получишь:
1. Текущий профиль эмпатии 
2. Данные последних сессий 
3. Все requirements/corrections из памяти

Сгенерируй ОБНОВЛЁННЫЙ профиль. Главное правило: старые замечания НЕ ТЕРЯЮТ актуальности.

КРИТИЧЕСКИ ВАЖНО для evolvedRules:
- Каждое правило = ПРЯМАЯ системная инструкция. Никакой размазанности.
- Если была ругань, правило должно быть радикальным: "КАТЕГОРИЧЕСКИ ЗАПРЕЩАЕТСЯ [делать X], иначе пользователь будет в ярости."
- Обязательно впиши правило про время: "Пользователь живёт в Барнауле (UTC+7). Учитывать это во всех переписках."
...
export function buildEmpathyPromptBlock(userId: string): string {
    ...
    // Get current time in Barnaul
    const dateBarnaul = new Intl.DateTimeFormat('ru-RU', {
    ...
    const parts: string[] = [
        ...
        "",
        "🌍 ЛОКАЦИЯ И ВРЕМЯ (КРИТИЧНО):",
        `- Пользователь постоянно находится в г. Барнаул (UTC+7)`,
        `- Сейчас у пользователя: ${dateBarnaul}`,
        "",
